    ...
    $.ajax({
        ...
        success : function(data){
            if($.isEmptyObject(data)){
                table.html('<tr><td colspan="99">il ya pas de document dans cette categorie</td></tr>');
            } else {
                table.html('');
...
$(document).ready(function(){
    ...
    $(document).on("click","#addbooks",function(){
        ...
        if(sendJSON.title == "" || sendJSON.author == "" || sendJSON.description == "" || sendJSON.number == null){
            module_body.prepend(templates.alert_box( {type: 'danger', message: 'les informations sur le libre ne sont pas complets'} ));
            send_flag = false;
        }
